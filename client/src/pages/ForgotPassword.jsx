...
import { PlayCircle, Mail, ArrowLeft, Loader2, CheckCircle2 } from 'lucide-react';
...
const ForgotPassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      sessionStorage.setItem('resetEmail', email); // Save for prefilling reset page
      setIsSuccess(true);
    } catch (error) {
        ...
        <div className="bg-[#0f0f11]/80 backdrop-blur-xl py-8 px-4 shadow-[0_0_40px_rgba(0,0,0,0.5)] sm:rounded-2xl sm:px-10 border border-white/5 relative overflow-hidden">
          ...
          {isSuccess ? (
            <div className="text-center space-y-6">
              ...
              <div>
                <h3 className="text-xl font-medium text-white">Cek Email Anda</h3>
                <p className="mt-2 text-gray-400">
                  Kami telah mengirimkan instruksi dan kode reset ke <span className="text-white">{email}</span>
                </p>
              </div>
              <div className="pt-4">
                <Link
                  to="/reset-password"
                  className="w-full flex justify-center py-3 px-4 border border-transparent rounded-xl shadow-sm text-sm font-semibold text-white bg-violet-600 hover:bg-violet-500 transition-all"
                >
          ...
          <div className="mt-6">
            <Link to="/login" className="flex items-center justify-center text-sm font-medium text-gray-400 hover:text-white transition-colors">
