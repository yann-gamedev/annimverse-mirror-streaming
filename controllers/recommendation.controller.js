...
exports.getPersonalizedRecommendations = async (req, res) => {
    try {
        ...
        if (isGroupB) {
            ...
            const collabResult = await exports.getCollaborativeRecommendations(req, res, true);
        ...
        const blacklistedIds = userDoc.blacklistedAnime ? userDoc.blacklistedAnime.map(id => id.toString()) : [];
        ...
        const allGenres = watchHistory.flatMap(h => h.anime.genres).filter(g => g);
        ...
        // Initial Scoring
        let scoredCandidates = candidates.map(anime => {
            const matching = anime.genres.filter(g => favoriteGenres.includes(g)).length;
            const baseScore = (matching / favoriteGenres.length) * 0.6 + (anime.rating / 10) * 0.4;
        ...
        while (finalRecommendations.length < limit && scoredCandidates.length > 0) {
            ...
            for (const h of watchHistory.slice(0, 20)) {
                ...
                const overlap = best.genres.filter(g => h.anime.genres.includes(g)).length;
                if (overlap > maxOverlap) { maxOverlap = overlap; reason = h.anime.title; }
            }
 ...
 */
exports.getCollaborativeRecommendations = async (req, res, internalCall = false) => {
    try {
        const userId = req.user._id;
        const limit = parseInt(req.query?.limit) || 20;

        // 1. Get user's history (just IDs)
        const userHistory = await History.find({ user: userId }).select('anime').sort({ watchedAt: -1 }).limit(20);
...
exports.getPersonalizedTrending = async (req, res) => {
    try {
        ...
        if (trendingCache.data) {
            ...
            const trendingData = await History.aggregate([
                { $match: { watchedAt: { $gte: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000) } } },
        ...
        const scoredTrending = trendingList.map(anime => {
            let score = anime._trendingData?.viewCount || 0;

            // Boost if genre matches user favorites
            const matchCount = anime.genres.filter(g => watchedGenres.includes(g)).length;
            if (matchCount > 0) {
                score *= (1 + (matchCount * 0.2)); // 20% boost per matching genre
            }

            // Remove if blacklisted
            if (user.blacklistedAnime && user.blacklistedAnime.includes(anime._id.toString())) {
                return null;
            }
...
exports.getWatchPartyRecommendations = async (req, res) => {
    try {
        ...
        const commonGenres = Object.keys(genreFrequency).filter(g => genreFrequency[g] >= threshold);

        // Fallback if no intersection: take top 3 global genres of the group
        const targetGenres = commonGenres.length > 0 ? commonGenres :
            Object.keys(genreFrequency).sort((a, b) => genreFrequency[b] - genreFrequency[a]).slice(0, 3);
