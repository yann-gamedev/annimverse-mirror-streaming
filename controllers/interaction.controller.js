...
exports.postComment = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Gagal kirim komentar", error: error.message });
  }
};
...
exports.addToHistory = async (req, res) => {
  try {
    ...
    const now = new Date();
    const lastDate = user.stats.lastWatchDate
      ? new Date(user.stats.lastWatchDate)
      : null;
