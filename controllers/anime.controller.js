...
exports.createAnime = async (req, res) => {
    try {
        const { title, slug, synopsis, genres, posterUrl, rating, status, releaseYear, totalEpisodes } = req.body;
        ...
        const newAnime = await Anime.create({
            title, slug, synopsis, genres, posterUrl, rating, status, releaseYear, totalEpisodes
        });

        res.status(201).json({ message: "Anime berhasil dibuat!", data: newAnime });
    } catch (error) {
        res.status(500).json({ message: "Gagal membuat anime", error: error.message });
    }
};
...
exports.createEpisode = async (req, res) => {
    try {
        ...
        console.log(`✅ Episode ${episodeNumber} berhasil ditambahkan ke ${anime.title}`);
