...
exports.createBadge = async (req, res) => {
    try {
        ...
        const { code, name, description, icon, category, requirementValue } = req.body;
        ...
        if (exists) {
            return res.status(400).json({ message: "Badge dengan kode ini sudah ada!" });
        }
        ...
        res.status(500).json({ message: "Gagal membuat badge", error: error.message });
    }
};
